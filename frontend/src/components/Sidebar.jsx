...
const Sidebar = ({ activeNav, setActiveNav, sidebarOpen, setSidebarOpen }) => {
  ...
  useEffect(() => {
    const fetchDoctor = async () => {
      ...
      try {
        const res = await axios.get(`${DOCTOR_API}/doctor?email=${currentEmail}`);
        setDoctor(res.data);
      } catch (err) {
        console.error(err);
      }
  ...
  useEffect(() => {
    if (["/appointment", "/followups", "/opd", "/history"].includes(location.pathname)) {
  ...
  const isAppointmentGroupActive =
    ["/appointment", "/followups", "/opd", "/history"].includes(location.pathname);
  ...
  return (
    <aside
      className={`
        ...
        w-64 h-screen bg-[#E6E6E6] flex flex-col px-3 py-6 overflow-y-auto
    ...
    >
      ...
      <button
        ...
        className="lg:hidden absolute top-4 right-4 p-1.5 rounded-lg hover:bg-gray-300 transition"
      >
      ...
      <div className="bg-white rounded-2xl p-5 flex flex-col items-center shadow-sm mb-6">
        ...
        <button
          ...
          onClick={() => { navigate("/doctor/edit"); setSidebarOpen?.(false); }}
        >
          Edit profile
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl p-2 shadow-sm flex-1">
        {/* Dashboard */}
        <button onClick={() => handleNav("Dashboard", "/dashboard")} className={navItemClass(isActive("/dashboard"))}>
        ...
        <div className="mb-1">
          ...
          {appointmentsOpen && (
            <div className="ml-4 mt-1 border-l-2 border-gray-200 pl-3 flex flex-col gap-0.5">
              {[
                { label: "Appointments", path: "/appointment", icon: <path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /> },
                { label: "Follow-ups",   path: "/followups",   icon: <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /> },
                { label: "OPD",          path: "/opd",         icon: <path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /> },
                { label: "History",      path: "/history",     icon: <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /> },
              ].map(({ label, path, icon }) => (
                <button
                  key={path}
                  ...
                  className={`w-full flex items-center gap-2 px-2 py-2 rounded-lg text-xs font-medium transition-all ${
                    isActive(path) ? "text-[#7FB53D] bg-green-50" : "text-gray-500 hover:bg-gray-50 hover:text-gray-700"
                  }`}
                >
        ...
        {/* Patients */}
        <button onClick={() => handleNav("Patients", "/patient")} className={navItemClass(isActive("/patient"))}>
