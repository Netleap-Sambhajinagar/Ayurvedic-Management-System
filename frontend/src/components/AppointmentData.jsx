...
const Badge = ({ label, color="green" }) => {
  ...
  return (
    <span className="badge" style={styles[color]||styles.gray}>{label}</span>
  );
};
                  ...
                  <tr key={p.id} className="trow" style={{ borderBottom:"1px solid rgba(39,103,73,.05)" }}>
                    ...
                    <td className="py-3 px-4">
                      <button className="text-sm font-medium hover:underline text-left" style={{ color:"var(--fern)" }}
                        onClick={() => navigate(`/patient/${p.id}`, { state: { from: viewType } })}>
                        {p.name}
                      </button>
...
export default function AppointmentData({ viewType="appointments" }) {
  ...
  const handleAppointed = async (pid, val) => {
    ...
    try { await axios.patch(`${PATIENTS_API}/${pid}/appointed`,{isAppointed:val}); await fetchAll(); }
  ...
  const apptCols = [
    ...
    { key:"isAppointed", label:"Cabin?",
      render:p=>p.isAppointed==="yes"?<Badge label="✓ Yes" color="green"/>:(
        <div className="flex gap-1.5">
          <button disabled={updating[p.id]} onClick={()=>handleAppointed(p.id,"yes")}
            className="text-xs px-3 py-1 rounded-lg font-medium text-white transition" style={{ background:"var(--fern)" }}>Yes</button>
          <button disabled={updating[p.id]} onClick={()=>handleAppointed(p.id,"no")}
            className="text-xs px-3 py-1 rounded-lg font-medium transition" style={{ background:"var(--parchment)", color:"var(--mist)", border:"1px solid var(--border)" }}>No</button>
        </div>
      )},
    { key:"actions", label:"",
      render:p=>(
        // Only show cancel if the appointment hasn't been completed (OPD not done)
        p.isOpd !== "yes" ? (
  ...
  const followupCols = [
    { key:"followupDate",     label:"Follow-up Date", render:p=><span className="font-medium" style={{color:"var(--terracotta)"}}>{fmt(p.followupDate)}</span> },
  ...
  return (
    <div className="flex-1 min-h-screen anim-page" style={{ background:"var(--parchment)" }}>
      ...
      <div className="px-6 sm:px-10 py-6">
        {viewType==="appointments" && <SectionTable title="Appointments" patients={filter(appointments)} columns={apptCols} emptyMsg="No appointments registered." loading={loading}/>}
        {viewType==="followups"    && <SectionTable title="Follow-ups" patients={filter(followups)} columns={followupCols} emptyMsg="No follow-ups scheduled." loading={loading}/>}
        {viewType==="opd" && <SectionTable
      ...
      {cancelModal && <CancelModal patient={cancelModal.patient} type={cancelModal.type}
