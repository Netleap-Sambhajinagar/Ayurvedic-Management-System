...
const BulkDeleteConfirmModal = ({ count, onConfirm, onCancel, deleting }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
    <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={onCancel} />
    <div className="card relative w-full max-w-sm p-6 z-10 anim-scale">
      <div className="flex items-center justify-center w-12 h-12 rounded-full mx-auto mb-4"
      ...
      <h3 className="text-xl font-light text-center mb-2" style={{ fontFamily: "var(--font-serif)", color: "var(--forest)" }}>
        Delete {count} Record{count !== 1 ? "s" : ""}?
      </h3>
      <div className="text-xs text-center px-3 py-2 rounded-xl mb-5"
        style={{ background: "rgba(193,105,79,.08)", color: "var(--terracotta)", border: "1px solid rgba(193,105,79,.2)" }}>
        ⚠️ This will permanently delete {count} selected visit record{count !== 1 ? "s" : ""}. Patient details will not be deleted.
      </div>
      <div className="flex gap-3">
        <button onClick={onCancel} disabled={deleting} className="btn-ghost flex-1 justify-center py-2.5">
          Cancel
...
export default function HistoryData() {
  ...
  const handleBulkDeleteConfirm = async () => {
    ...
    try {
      await axios.delete(`${PATIENTS_API}/visits`, { data: { ids: Array.from(selected) } });
      setHistory(prev => prev.filter(v => !selected.has(v.id)));
      const count = selected.size;
      setSelected(new Set());
      setBulkMode(false);
      setShowBulkConfirm(false);
      showToast(`${count} record${count !== 1 ? "s" : ""} deleted successfully.`);
    } catch (err) {
      showToast(err?.response?.data?.error || "Failed to bulk delete records.", "error");
    } finally {
  ...
  const allFilteredIds = filtered.map(h => h.id);
  ...
  return (
    <div className="flex-1 min-h-screen anim-page" style={{ background: "var(--parchment)" }}>
      ...
      <div className="px-6 sm:px-10 py-5 border-b flex items-center justify-between gap-4 flex-wrap"
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className="text-xs py-1.5 px-3 rounded-xl font-medium transition"
            style={{
              color: bulkMode ? "white" : "var(--fern)",
              background: bulkMode ? "var(--fern)" : "rgba(39,103,73,.08)",
              border: "1px solid rgba(39,103,73,.25)"
            }}>
      ...
      {bulkMode && selected.size > 0 && (
        <div className="px-6 sm:px-10 py-3 flex items-center gap-3 border-b"
          style={{ background: "rgba(193,105,79,.06)", borderColor: "rgba(193,105,79,.2)" }}>
          <span className="text-sm font-medium" style={{ color: "var(--terracotta)" }}>
            {selected.size} record{selected.size !== 1 ? "s" : ""} selected
          </span>
          <button
            onClick={() => setShowBulkConfirm(true)}
            className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-xl font-semibold text-white transition"
            style={{ background: "linear-gradient(135deg,#8b2a2a,var(--terracotta))" }}>
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round"
                d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
            </svg>
          ...
          <button onClick={() => setSelected(new Set())} className="text-xs" style={{ color: "var(--mist)" }}>
            Clear selection
          </button>
        </div>
      )}
          ...
          <>
            ...
            <div className="card overflow-hidden anim-up hidden md:block">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filtered.map((h, i) => (
                      <tr key={h.id || i} className="trow" style={{ borderBottom: "1px solid rgba(39,103,73,.05)", background: selected.has(h.id) ? "rgba(39,103,73,.04)" : undefined }}>
                        {bulkMode && (
            ...
            <div className="card overflow-hidden anim-up md:hidden">
              <div className="divide-y" style={{ borderColor: "rgba(39,103,73,.06)" }}>
                {filtered.map((h, i) => (
                  <div key={h.id || i} className="p-4" style={{ background: selected.has(h.id) ? "rgba(39,103,73,.04)" : undefined }}>
