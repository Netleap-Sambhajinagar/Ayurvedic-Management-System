    ...
    try {
        ...
        for (const doc of doctors) {
            await mysqlConn.execute(
                ...
                [doc.name, doc.email, doc.specialization, doc.password, doc.createdAt || new Date(), doc.updatedAt || new Date()]
            );
        }
        ...
        for (const p of patients) {
            await mysqlConn.execute(
                ...
                [
                    p.email, p.name, p.age || 0, p.weight || 0, p.height || 0, p.contactNo || "",
                    p.bodyBuild || null, p.skinType || null, p.digestion || null, p.hungerPattern || null,
                    p.sleepPattern || null, p.bowelMovements || null, p.stressResponse || null, p.energyLevel || null,
                    p.vikritiType || null, p.severity || null, p.createdAt || new Date(), p.updatedAt || new Date()
                ]
            );
        }
